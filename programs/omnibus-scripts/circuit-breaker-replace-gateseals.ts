...
import { encodeFromAgent, getCallScriptAction, votingNewVote } from '@scripts';
import {
  CallScriptAction,
...
export const circuitBreakerReplaceGateseals = async () => {
  const agentForwardedCalls: CallScriptAction[] = [];
  ...
  for (const gateSealAddress of GATE_SEAL_ADDRESSES) {
    ...
    for (const sealable of sealables) {
      ...
      const revokeInner = await getCallScriptAction(sealableContract, 'revokeRole', [pauseRoleHash, gateSealAddress]);
      const [, revokeCall] = encodeFromAgent(revokeInner);
      agentForwardedCalls.push(revokeCall);
      descriptions.push(`${step++}. Revoke PAUSE_ROLE from GateSeal ${gateSealAddress} on ${sealable}`);

      // Grant PAUSE_ROLE to CircuitBreaker
      const grantInner = await getCallScriptAction(sealableContract, 'grantRole', [pauseRoleHash, CIRCUIT_BREAKER]);
      const [, grantCall] = encodeFromAgent(grantInner);
      agentForwardedCalls.push(grantCall);
      ...
      const registerInner = await getCallScriptAction(circuitBreakerContract, 'registerPauser', [sealable, sealingCommittee]);
      const [, registerCall] = encodeFromAgent(registerInner);
      agentForwardedCalls.push(registerCall);
      descriptions.push(`${step++}. Register ${sealable} with CircuitBreaker, pauser: ${sealingCommittee}`);
    }
  }
  ...
  // Wrap as a Dual Governance proposal
  const dgExternalCalls = agentForwardedCalls.map((call) => ({
    target: call.to,
    value: 0,
    payload: call.data,
  }));
  ...
  const dgCall: CallScriptAction = {
    ...
    data: dualGovernanceContract.interface.encodeFunctionData('submitProposal', [dgExternalCalls, '']),
  };
